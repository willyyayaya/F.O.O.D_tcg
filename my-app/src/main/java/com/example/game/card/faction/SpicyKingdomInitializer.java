...
public class SpicyKingdomInitializer implements FactionCardInitializer {
    ...
    private void initializeCharacterCards(Map<String, Card> allCards, List<CharacterCard> allCharacters) {
        ...
        CharacterCard hotPot = new CharacterCard(
            ...
            Rarity.GOURMET_DELIGHT, 4, 6, true, Faction.SPICY_KINGDOM);
        ...
        CharacterCard chiliWarrior = new CharacterCard(
            "辣椒戰士", 2, "【開胃】：對一個敵方角色造成1點傷害。【連擊】：攻擊後，有30%機率再次攻擊。", 
            Rarity.CASUAL_BITES, 2, 3, true, Faction.SPICY_KINGDOM);
        allCards.put(chiliWarrior.getName(), chiliWarrior);
        allCharacters.add(chiliWarrior);
        
        CharacterCard pepperMage = new CharacterCard(
            "胡椒法師", 3, "【開胃】：使一個敵方角色獲得-1/-1。【連擊】：攻擊後，有40%機率再次攻擊。", 
            Rarity.GOURMET_DELIGHT, 2, 4, true, Faction.SPICY_KINGDOM);
        allCards.put(pepperMage.getName(), pepperMage);
        allCharacters.add(pepperMage);
        
        CharacterCard spiceMaster = new CharacterCard(
            "香料大師", 6, "【開胃】：對所有敵方角色造成2點傷害。【連擊】：攻擊後，有50%機率再次攻擊。", 
    ...
    private void initializeSpellCards(Map<String, Card> allCards, List<SpellCard> allSpells) {
        ...
        SpellCard spicyExplosion = new SpellCard(
            "辣味爆炸", 3, "對所有敵方角色造成2點傷害，並使其獲得【灼燒】效果：每回合受到1點傷害。", 
    ...
    private void initializeFieldCards(Map<String, Card> allCards, List<FieldCard> allFieldCards) {
        ...
        FieldCard chiliField = FieldCard.createEnvironmentField(
            "辣椒田", 2, "使一個友方角色獲得【連擊】效果：攻擊後，有30%機率再次攻擊。", 
            Rarity.CASUAL_BITES, 2, Faction.SPICY_KINGDOM);
        ...
        FieldCard pepperPalace = FieldCard.createEnvironmentField(
            "胡椒宮殿", 5, "友方角色獲得+1/+1，且每回合開始時對一個敵方角色造成1點傷害。", 
            Rarity.CULINARY_HERITAGE, 4, Faction.SPICY_KINGDOM);
        ...
        FieldCard spiceSanctuary = FieldCard.createEnvironmentField(
            "香料聖殿", 6, "所有友方角色獲得【連擊】效果，且每回合開始時對所有敵方角色造成1點傷害。", 
